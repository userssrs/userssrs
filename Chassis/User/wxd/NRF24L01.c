...
//初始化NRF24L01IO口
void NRF24L01_task(void const *pvParameters)
{
 Init_NRF24L01();
  // SCK_L;
  if (NRF24L01_Check())
  {
	...
	for (;;)
	{
	uint8_t TxData[32] = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2};

    if (NRF24L01_RxPacket(TxData) == 0) // 接收到数据
    {
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_13, GPIO_PIN_SET);
    }
    pitch1 = (int8_t)TxData[1];
    roll1 = (int8_t)TxData[2];
    yaw1 = (int8_t)TxData[3];
    vx = (int8_t)TxData[4];
    vy = (int8_t)TxData[5];
    vw = (int8_t)TxData[6];
